	...
	if(quantity > 0)
	{
		...
		else
		{
			if(quantity >= 11 && quantity <= 15)
			{
				...
				document.getElementById('results').innerHTML="<h4><ul><li><b>Supplier 1</b> can fulfill the request for " + priceDiscount1 + "€. This is the cheapest one so it should be highlighted.<br></li><br><li><b>Supplier 2</b> is not prompted because it does not have enough stock quantity available.</li><br><li><b>Supplier 3</b> can fulfill the request for " + priceNoDiscount3 + "€. If you order in September " + priceDiscount3 +"€. Fast delivery.</ul></h4>";
			}
			else
			{
				if(quantity >= 7 && quantity <= 10)
				{
					let priceDiscount1 = price1 - (price2 * 0.10);
					let priceDiscount2 = price2 - (price2 * 0.05);
					let priceNoDiscount3 = price3;
